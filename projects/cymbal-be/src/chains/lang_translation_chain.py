...
from prompts.lang_translation_prompt import LangTranslationPrompt
import logging
from config.config import configcontex
from langchain.llms.vertexai import _VertexAICommon


class LangTranslationChain:
    """Creates a chain for language translation based on VertexAI, with logging."""

    def __init__(
        self,
        llm: Optional[_VertexAICommon] = None,
        lang_translation_prompt: Optional[LangTranslationPrompt] = None,
        ...
        """
        The constructor for the LangTranslationChain class.
        """
        ...
        self.lang_translation_prompt = (
            lang_translation_prompt.create_lang_translation_prompt()
            if lang_translation_prompt
            else LangTranslationPrompt().create_lang_translation_prompt()
        )
    ...
    def log(self, level, message):
        self.logger.log(
            level, f"LangTranslationChain - Origin: {request_origin.get()} | {message}"
        )
    ...
    def initialize_llm_chain(self, llm_chain: Optional[LLMChain]):
        ...
        try:
            self.llm_chain = (
                llm_chain
                if llm_chain
                else LLMChain(llm=self.llm, prompt=self.lang_translation_prompt)
            )
    ...
    def process_query(self, query: str) -> Union[Dict[str, str], str]:
        ...
        if not llm_response:
            self.log(
                logging.ERROR, "LLM-Response Error, LangTranslationChain Stage Response is empty."
            )
            raise ValueError(
                "LLM-Response Error, LangTranslationChain Stage Response cannot be empty or None"
            )
